 ...
 * First checks for token in Authorization header, if not found, checks for token in cookies
 */
export const unifiedAuthMiddleware = (req: Request, res: Response, next: NextFunction): void => {
    try {
        ...
        logger.info(`Unified auth validation attempt from IP: ${req.ip}, Path: ${req.path}, Token source: ${tokenSource || 'none'}`);
        ...
        logger.info(`Unified auth validation successful - UserID: ${userData.id}, Email: ${userData.email}, IP: ${req.ip}, Source: ${tokenSource}`);

        next();

    } catch (error) {
        logger.error(`Unified auth middleware error - IP: ${req.ip}, Error: ${error instanceof Error ? error.message : 'Unknown error'}`);
...
/**
 * Optional Unified Authentication Middleware
 * Similar to unifiedAuthMiddleware but doesn't require authentication
 * Will set req.user if valid token is found, but continues regardless
 */
export const optionalUnifiedAuthMiddleware = (req: Request, res: Response, next: NextFunction): void => {
    try {
        let token: string | null = null;
        let tokenSource: string = '';
        ...
        if (!token) {
            logger.info(`Optional unified auth: No token provided - IP: ${req.ip}, Path: ${req.path}`);
            next();
            return;
        }
        ...
        if (decoded && decoded.id && decoded.email) {
            ...
            logger.info(`Optional unified auth validation successful - UserID: ${decoded.id}, Email: ${decoded.email}, IP: ${req.ip}, Source: ${tokenSource}`);
        } else {
            logger.info(`Optional unified auth: Invalid token provided - IP: ${req.ip}, Source: ${tokenSource}`);
        }

        next();

    } catch (error) {
        logger.warn(`Optional unified auth middleware error - IP: ${req.ip}, Error: ${error instanceof Error ? error.message : 'Unknown error'}`);
        next();
    }
};
